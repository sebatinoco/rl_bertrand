...
class LinearBertrandEnv():
    ...
    def demand(self, prices):
        ...
        quantities = [q_min if p == p_min and p < self.A_t else 0 for p in prices]
    ...
    def get_inflation(self):
        ...
        if (sample < self.rho) & (self.gen_inflation):
            ...
            self.pi_N = (self.pN - self.c_t) * self.demand([self.pN])[0]
            self.pi_M = (self.pM - self.c_t) * self.demand([self.pM])[0]
