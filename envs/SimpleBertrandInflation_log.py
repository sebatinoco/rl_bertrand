...
class BertrandEnv():
    def __init__(self, N, k, rho, A = 3, e = 1, c = 1, v = 3, xi = 0.2):
        ...
        self.A = np.log(A) # highest disposition to pay
    ...
    def reset(self):
        ...
        self.pN, self.pM = np.log(self.pN), np.log(self.pM)
        ...
        self.price_high, self.price_low = np.log(self.price_high), np.log(self.price_low)
        
        past_prices = [list(np.log(prices)) for prices in self.prices_space.sample()] # init prices
    ...
    def demand(self, prices):
        ...
        prices = np.exp(prices) # exp prices
        p_min = np.min(prices)
        q_min = np.exp(self.A) - p_min * np.exp(self.e)
    ...
    def get_inflation(self):
        ...
        if sample < self.rho:
            ...
            self.c += np.log(1 + inflation_t) # adjust marginal cost
            self.A += np.log(1 + inflation_t) # adjust pay disposition
            
            #self.c *= (1 + inflation_t) # adjust marginal cost
            #self.A *= (1 + inflation_t) # adjust pay disposition
            
            #print('Calculating new equilibria...')
            self.pN = np.exp(self.c) # get nash price
            self.pM = (np.exp(self.A) + np.exp(self.c) * self.e) / (2 * self.e) # get monopoly price
            
            self.pi_N = (self.pN - self.c) * self.demand([self.pN])[0]
            self.pi_M = (self.pM - self.c) * self.demand([self.pM])[0]
            assert self.pi_M > self.pi_N, 'monopoly profits should be higher than nash profits'
            
            self.price_high = np.log(self.pM * (1 + self.xi))
            self.price_low = np.log(self.pN * (1 - self.xi))
