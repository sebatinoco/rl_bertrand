...
class SACAgent:
    ...
    def __init__(self, dim_states, dim_actions, action_low, action_high, moving_dim, max_var = 0.2, hidden_size = 256, 
                 gamma = 0.99, tau = 0.01, alpha = 0.2, Q_lr = 3e-4, actor_lr = 3e-4, alpha_lr = 3e-4):
    ...
    def rescale_action(self, action):
        ...
        #assert type(action) == float, f'action is not float: {action}'
        #assert type(scaled_action) == float, f'scaled action is not float: {scaled_action}'
        #assert type(self.moving_avg) == float, f'moving avg is not float: {self.moving_avg}'
        ...
        #self.moving_avg += (self.scaled_history[self.idx+self.moving_dim] - self.scaled_history[self.idx]) / self.moving_dim
