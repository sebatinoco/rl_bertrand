...
class Actor(nn.Module):
    def __init__(self, input_size, output_size, action_high, action_low, hidden_size = 256, num_layers = 2, dropout = 0.1):
        super().__init__()
...
class DDPGAgent():
    def __init__(self, N, action_high, action_low, 
                 actor_lr = 1e-3, Q_lr = 1e-2, gamma = 0.99, tau = 0.9, Q_updates = 1):
        
        # actor and critic
        self.actor = Actor(N + 1, 1, action_high, action_low)
        self.Q = QNetwork(N + 1, 1)
        
        # actor and critic targets
        self.actor_target = Actor(N + 1, 1, action_high, action_low)
        self.Q_target = QNetwork(N + 1, 1)
